...
识别特征:`archmap dirty` 出现大量无主文件,且 `archmap status` 锚点大面积失效——这是物理布局变了,架构没变。

1. 看 dirty 输出的**「迁移线索」**:每行是"旧路径(原属模块 X)→ 新路径",照此映射写锚点;
2. 草稿用 `kind: refactor`,patch 只放 `update_anchors`,story 一句话说明移动原因(如"src 平铺改分层");
3. **严禁趁机修改模块名称/职责/关系**——物理移动只更新锚点;确有职责调整的,另开一张快照;
4. 提交前跑 `archmap status` 确认失效锚点清零、覆盖率恢复。
